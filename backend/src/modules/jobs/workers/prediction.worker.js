...
// (which owns Socket.IO) can relay them â€” works from both the in-process
...
const worker = new Worker(
  ...
  async (job) => {
    ...
    publishSocketEvent('socket:job:progress', batchJobId, {
      processed_count: updatedJob.processed_count,
      total_containers: updatedJob.total_containers,
  ...
  },
  {
    ...
    // concurrency=3: 3 chunks Ã— 50 containers = 150 in-flight, sequential DB queries per chunk
    concurrency: 3,
  }
);
